...
export const AboutDevelopment = () => {
  const contents = [
    ...
    },
    {
      ...
      content:
        '個人制作として、Next.jsを用いた『色彩感覚トレーニング』と題したWebアプリケーションを開発しました。NextAuthを使用したFirebaseとGoogle認証との連携、色彩選択ライブラリを導入したインタラクティブなUIの実装を行いました。\n状態管理にはrecoilやvaltioを使用します。再レンダリングを起こしたくない場合はRefオブジェクトをコンテキスト化するなど、パフォーマンスチューニングを行います。\nただ、フロントエンドテストやstorybookでのコンポーネント開発管理といった部分は未経験なので、この点は作成したアプリケーションのアップデートと共に学習していきたいです。\nまた、SSGとSSRの使い分けなどはまだ深く理解できていないので、今後の課題となります。',
    },
    {
      title: 'WordPress',
      content:
